...
## [2.13.3] - 2026-09-07
### Fixed
- **Xbox Library now shows the TU that is actually installed and in use.** Three accuracy bugs in the title-update display: (1) `normalizeTUVersions` stamped `Active = true` on the highest-version row of the merged installed+candidate list regardless of install state, so "Active" could appear on a TU that wasn't even on the console - the flag now comes only from the FTP scan's `.disabled` suffix, which is what the console actually loads; (2) version parsing only matched filenames that *start* with `tu`, so real console TU files like `545408A7_TU27_0C48794E.bin` (the exact name GODSend itself uploads for XboxUnity TUs) or Horizon-style `TU27…` showed as a bare "Title Update" with no version and couldn't be deduped against the XboxUnity candidate list - `extractTUVersion` now picks up `tu<digits>` anywhere in the name; (3) freshly installed TUs left every pre-existing bare TU file active (any bare file scans as "active"), so two rows could both read "Active" - both upload paths (direct URL and Minerva torrent) now deactivate sibling TUs so the new file is the single active one. Regression tests in `services/content/content_test.go`.

## [2.13.2] - 2026-09-03
### Fixed
- **XDVDFS directory parsing no longer stops at the first 0-byte file.** `parseDirSector` treated a directory entry with `size == 0` as the end of the table, but a zero-byte file is a perfectly legal entry - and because an XDVDFS directory table is an AVL tree written root-first, an empty file whose name sorts near the middle lands at offset 0 and hid *every* sibling. GTA V's Disc 1 (Install) has exactly that: a 0-byte `disc1.rsn` sitting alphabetically in the middle of its nine root entries, so the root directory read back as empty and both `default.xex` and `content/` went missing. That surfaced first as `TitleID=545408A7 disc=?/? (no executable on disc)` and then as a hard `Content extract: contentExtract: no content/ dir in ISO` (issue #4), despite the disc plainly carrying `content/0000000000000000/545408A7/00000002/`. The walk now ends on a zero-length *name* (zero-filled padding) or the existing `0xFFFF` sector tail padding, and passes zero-size entries through as real files. Regression test in `utils/iso2god_dirtable_test.go`.
- **Content install can no longer report success after extracting nothing.** Making zero-size entries visible also made zero-size *directories* visible for the first time, and the four "first directory found" fallbacks in `ExtractXDVDFSContentToDir` and `ProbeContentPackageTitleID` (used when a disc's `Content/0000000000000000/<TitleID>/00000002` folders aren't named as expected) took the first `isDir()` entry unconditionally. An empty directory has no table to read, so `xdvdfsExtractRecursive` would walk nothing and return success - the pipeline would then FTP-transfer or 7z-package an empty folder and tell the user the install completed. Those fallbacks now require `size > 0`.
...
## [2.12.32] - 2026-08-29
### Fixed
- **FTP transfers of multi-package digital content no longer drop files.** `ProcessDigital` (Internet Archive-sourced XBLA/DLC/XBLIG installs) and `ProcessMinervaDigital` (the same install type sourced from Minerva/BitTorrent, the default source) both stopped scanning the extracted archive at the first file with a valid Xbox content header, so an archive bundling more than one package (base game + title update, or several DLC packs) only ever transferred the first one - reported as "FTP transfers do not complete" (issue #3): a 608 MB download would FTP a single 32 MB file and finish. `findDigitalContentFiles` now collects every valid content file in the archive; the shared `transferDigitalContentFilesFTP` uploads all of them over one connection, each to its own `titleID/content-type` folder (derived per-file from that file's own header, since a bundle can mix content types), with correct `[n/total]` progress and remote directories created up front so a mid-transfer reconnect can't leave a folder missing. The non-FTP local-download path still installs only one file - the Aurora `type=raw` manifest/installer only support a single file per game - but now picks the largest package instead of whichever sorts first lexically, so a title update or DLC pack can no longer be installed in place of the base game, and the status line reports how many packages were left behind.
...
- **Aurora malformed-DB logging.** DB buffers pulled over FTP are now validated before `sql.js` parses them (new `infrastructure/auroraDbDiagnostics.ts`); a corrupt or truncated DB produces an actionable error naming the DB and the likely cause (truncated download vs mid-write vs not-a-DB) with sha256, instead of the opaque "database disk image is malformed".

### Fixed
- **Aurora "database disk image is malformed" no longer kills the library.** A reporter's console had genuine on-disk corruption in `settings.db` (`UserRecentGames` b-tree page rowids out of order, a never-used page, broken autoindex) while `content.db` stayed intact - the game LIST lives in `content.db`, but the single corrupt metadata query threw and failed the whole Aurora/Xbox library load. The three `settings.db` metadata tables (UserHidden / UserFavorites / UserRecentGames) are now read tolerantly: a corrupt table logs a WARN with the full `PRAGMA integrity_check` signature (page/rowid/index - diagnosable without the file) and degrades to empty, so the library still loads all games with `lastPlayed`/`timesPlayed` empty until the DB is rebuilt in Aurora on the console. Confirmed the header was clean (not truncation, not WAL-mode hot-copy) - this was Aurora's own corruption, not an FTP transfer bug.
- **FTP GOD transfer infinite retry loop.** Aurora's FtpDll does NOT overwrite files: `STOR` of an already-existing filename returns `550 Could not create file`. An interrupted GOD transfer left already-uploaded files behind, so every retry re-`STOR`ed from file 1, hit 550, and the whole game looped forever on a 5-minute backoff. `infrastructure/ftp/client.go` `UploadFile` now checks `FileSize` before `STOR`: equal size means already uploaded (skip, count toward progress), a size mismatch means a partial leftover (delete then re-`STOR`), absent means `STOR` as usual. Already-complete files are never re-uploaded; only a partial file is redone.
- **Debrid hardening (key-leak, wrong-file, account hygiene).** Folded in from `security-auditor` and `code-reviewer` passes over the Debrid feature. (1) **TorBox API key no longer leaks into logs**: TorBox's `requestdl` puts the token in the URL query string, and on an HTTP failure Go's `*url.Error` embeds that URL in the error, which `Logf` would have written to the app log. `infrastructure/debrid/debrid.go` `doJSON` now redacts the token (raw and percent-encoded) from any error before it propagates, so the "keys never logged" rule holds. (2) **Real-Debrid / TorBox no longer download the wrong file on a name-match miss**: when the target file basename can't be matched inside a multi-file collection torrent, both providers now log a WARN and fall back to the native torrent/aria2c source instead of silently selecting "all" (RD) or the largest file (TorBox) and returning a direct link for an arbitrary file. (3) **TorBox cleans up abandoned items on fall back**: a timeout or error on `createtorrent`/`createwebdownload` now best-effort deletes the queued item (mirroring Real-Debrid's existing `deleteTorrent`), so a cache miss no longer leaves a dead entry on the account. RD cleanup errors are now logged at WARN. New unit tests cover the TorBox timeout-cleanup and both providers' no-match fall back.
- **Config file now owner-only (`0o600`).** `src/electron-app/services/settingsService.ts` `writeConfig` writes the app config with mode `0o600`; it holds the IA cookie and the Debrid API keys and was previously world-readable.
- **IA session headers no longer leak to lookalike hosts.** `app/config.go` `ApplyArchiveOrgHeaders` matched `archive.org` as a substring (so `evilarchive.org` or `archive.org.attacker.com` would have received the IA Cookie/Authorization). It now uses a label-suffix host check (`archive.org` or `*.archive.org`, port-stripped).
- **Debrid / IA secrets scrubbed from the aria2c child environment.** `infrastructure/torrent/torrent.go` now sets `cmd.Env` for the aria2c child to the parent env minus any `*_KEY`/`*_SECRET`/`*_TOKEN`/`*_PASSWORD`/`*_COOKIE`/`*_AUTHORIZATION` var, so the Debrid keys and IA cookie can't leak via the child process environ (crash dump, `/proc/<pid>/environ`). PATH/HOME and proxy vars are unaffected.
