...
- **What it covers:** Minerva torrents on both providers; Internet Archive downloads on TorBox only, and only public IA items (Real-Debrid cannot proxy archive.org URLs). The Xbox 360 / Xbox IA collections are login-gated (`access-restricted-item`), so TorBox can't fetch them (its scanner hits the URL unauthenticated and gets a 403); those items skip TorBox and download directly with your archive.org cookie. Only one provider is active at a time.
...
- **TorBox limits on Minerva:** TorBox caches the whole torrent with no per-file selection, so a Minerva collection torrent larger than its ~200 GB per-torrent cap is skipped (the download falls back to P2P) rather than burning a creation slot. Every Minerva platform except Games Archive exceeds that cap, so on TorBox only Games Archive is debrid-accelerated. When TorBox is active and you browse one of those uncached sections, the Browse page shows an amber notice recommending Real-Debrid (which selects per file and so can cache any Minerva collection). A `checkcached` pre-check also lets an already-cached torrent skip the 60s poll, and a definitive cache miss falls back immediately instead of waiting.
